...
export default function PlayClient() {
  ...
  // Fetch list of questions: prefer the room's selected questions (if any)
  // and fall back to the upstream problems list.
  useEffect(() => {
    let mounted = true;
    async function load() {
      try {
        ...
        if (roomCode) {
          const resRoom = await fetch(`/api/rooms/${roomCode}/questions`);
          const jsonRoom = await resRoom.json();

          if (resRoom.ok && Array.isArray(jsonRoom?.questions)) {
            const qs = jsonRoom.questions;
            if (!mounted) return;
            setQuestions(qs);
        ...
        // Fallback: load from global problems API
